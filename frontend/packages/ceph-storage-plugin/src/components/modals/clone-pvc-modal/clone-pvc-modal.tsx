...
const ClonePVCModal = withHandlePromise((props: ClonePVCModalProps) => {
  ...
  const results: DataPoint[] = getInstantVectorStats(response, null, humanizeBinaryBytes);
  const pvcUsedCapacity: string = _.get(results[0], 'label')
    ? _.get(results[0], 'label')
    : 'No Data';
