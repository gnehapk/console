...
const NodeTableRow: React.FC<NodeTableRowProps> = ({ obj: node, index, key, style, customData, onSelect, setRowData }) => {
  const roles = getNodeRoles(node).sort();
  const devicesCount = configMapsForAllNodes[node.metadata.name] || 0;
  //const [setRowData, onSelect] = useSelect(); 
  console.log(customData, 'customDataprev');
  // React.useEffect(() => {
...
const NodesList: React.FC = props => {
  ...
  return (
    <Table 
      ...
      Row={(nodeProps) => <NodeTableRow {...props} onSelect={onSelect} setRowData={setRowData} />} 
      aria-label="Nodes" 
      virtualize 
...
export const CreateOCSServiceForm: React.FC<CreateOCSServiceFormProps> = React.memo((props1) => {
    ...
    <div className="ceph-ocs-install__form co-m-pane__body co-m-pane__form">
      ...
      <form className="co-m-pane__body-group" onSubmit={submit}>
        <fieldset>
          ...
          {!ipiInstallationMode && <ListPage kind={NodeModel.kind} showTitle={false} ListComponent={props => <Table customData={props.data} {...props} Header={NodeTableHeader} Row={(nodeProps) => <NodeTableRow {...nodeProps} onSelect={onSelect} setRowData={setRowData} />} aria-label="Nodes" virtualize onSelect={onSelect} />} />}
          {!ipiInstallationMode && <ListPage kind={NodeModel.kind} showTitle={false} ListComponent={props => <Table customData={props.data} {...props} Header={NodeTableHeader} Row={(nodeProps) => <NodeTableRow {...nodeProps} onSelect={onSelect} setRowData={setRowData} />} aria-label="Nodes" virtualize onSelect={onSelect} />} />}
        </fieldset>
