...
import { NO_PROVISIONER, LSO_NAMESPACE } from '../../../constants';
...
export const CreateOCS = withHandlePromise<CreateOCSProps & HandlePromiseProps>((props) => {
  ...
  const handleStorageClass = (sc: StorageClassResourceKind) => {
    ...
    k8sList(LocalVolumeSetModel, { ns: LSO_NAMESPACE })
      .then((list) => {
        const [lvs] = list.filter((l) => l?.spec?.storageClassName === sc?.metadata?.name);
        setFilteredNodes(
