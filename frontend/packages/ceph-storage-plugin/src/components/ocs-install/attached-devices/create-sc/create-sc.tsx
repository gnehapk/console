...
import { k8sCreate, k8sPatch, NodeKind, K8sResourceKind } from '@console/internal/module/k8s';
...
const CreateSC: React.FC<CreateSCProps> = ({ match }) => {
  ...
  const getDisabledCondition = (activeStep: WizardStep) => {
    if (activeStep.id === CreateStepsSC.DISCOVER)
      return (
        getNodes(state.showNodesListOnADV, state.allNodeNamesOnADV, state.nodeNamesForLVS).length <
        1
      );
    if (activeStep.id === CreateStepsSC.STORAGECLASS) {
