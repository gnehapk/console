...
import { PromiseComponent, PromiseComponentState} from '@console/internal/components/utils/promise-component';
...
class IPIDeployementModal extends PromiseComponent<ipiModalProps, PromiseComponentState> {
  ...
  render() {
    return <form onSubmit={this.submit} name="form" className="modal-content modal-content--no-inner-scroll">
      <ModalTitle>Create OCS Service</ModalTitle>
      <ModalBody>3 new nodes and a AWS bucket will be created in order to create the OCS Service.</ModalBody>
      <ModalSubmitFooter errorMessage={this.state.errorMessage} inProgress={this.state.inProgress} submitText='Create' cancel={this.props.cancel} cancelText='Cancel' />
    </form>;
  };
}
