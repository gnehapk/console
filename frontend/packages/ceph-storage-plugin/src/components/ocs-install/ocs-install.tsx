...
import { BreadCrumbs, RequestSizeInput, SelectorInput } from '@console/internal/components/utils/index';
...
export const CreateOCSServiceForm: React.FC<CreateOCSServiceFormProps> = React.memo((props) => {
  ...
  const OCSNodeList = React.useCallback(props => (
    <NodeList
      {...props}
      data={props.data}
  ...
  const submit = (event: React.FormEvent<EventTarget>) => {
    ...
    let labelPath = '/metadata/labels';
    let promises = [];

    //labeling the selected nodes
    promises.push(selectedNodeData.map(function (node) {
      let labels = SelectorInput.arrayify(_.get(node, labelPath.split('/').slice(1)));
      let patch = {
        op: labels.length ? 'replace' : 'add',
        value: { ..._.get(node, labelPath), 'cluster.ocs.openshift.io/openshift-storage':''},
        path: labelPath,
      };
    ...
    promises.push(selectedNodeData.map(function (node) {
      let patch = {
        taints: [...node.spec.taints, {key: '', value: '', effect: 'NoSchedule'}],
        path: '/spec/taints',
