...
export const NodeList: React.FC<NodeListProps> = (props) => {
  ...
  const onSelect = (event, isSelected, virtualRowIndex, data) => {
    ...
    let newSelectedNodeData = _.cloneDeep(selectedNodeData);
    ...
    _.each(data, (row, index: number) => {
      if (index < newSelectedNodeData.length) {
        // preserve existing selection state
        newSelectedNodeData[index] = {
          ...row,
          uid: row.metadata.uid,
          selected: newSelectedNodeData[index].selected,
        };
      } else {
        // set initial selection state from storage here if necessary...for now, initialize it false
        newSelectedNodeData.push({ ...row, uid: row.metadata.uid, selected: false });
      }
    });

    if (virtualRowIndex !== -1) {
      // set the selection based on virtualRowIndex node, it should exist in the array now
      newSelectedNodeData[virtualRowIndex].selected = isSelected;
    } else {
      // selectAll
      newSelectedNodeData = _.map(newSelectedNodeData, (row) => ({ ...row, selected: isSelected }));
    }
    setSelectedNodeData(newSelectedNodeData);
    props.onSelect(newSelectedNodeData);
  };
    ...
    <div className="ceph-ocs-install__form co-m-pane__body co-m-pane__form">
      <h1 className="co-m-pane__heading co-m-pane__heading--baseline">
        ...
        <div className="co-m-pane__heading-link">
          <button className="btn btn-link" onClick={props.changeToYAMLMethod}>
            Edit YAML
          </button>
        </div>
      </h1>
