...
export const selectWorkerRows = async () => {
  ...
  if (isAllSeleted === true) await click(currentSelectors.selectAllBtn);
  const workersAZ = [];
  const table = $('[aria-label="Node Table"]');
  ...
  rows.each(async (row, index) => {
    await (async () => {
      ...
      if (selectedNodes.length < 3) {
        workersAZ.push(splitedRow[4]);
        const checkBox = await table.$$('tbody input').get(index);
        const nodeName = await table
          .$$('tbody a')
          .get(index)
          .getText();
        await click(checkBox);
        await browser.wait(until.elementToBeSelected(checkBox));
