...
describe('Check add capacity functionality for ocs service', () => {
  describe('For common test cases', () => {
    beforeAll(async () => {
      ...
      expansionObjects.uid = _.get(expansionObjects.clusterJSON, 'metadata.uid').toString();
      ...
      expansionObjects.defaultSC = execSync(
        ...
        .toString()
        .trim();
      await click(getSCOption(expansionObjects.defaultSC));
      verifyFields();
...
if (TEST_PLATFORM === 'OCS') {
  ...
  if (clusterStatus && cephHealth) {
    describe('Check add capacity functionality for ocs service', () => {
      beforeAll(async () => {
        const statusCol = storageClusterRow(expansionObjects.uid).$('td:nth-child(3)');
        ...
        await browser.wait(
          until.textToBePresentInElement(
            statusCol.$('span.co-icon-and-text'),
            CLUSTER_STATUS.PROGRESSING,
          ),
        );
        await browser.wait(
          until.textToBePresentInElement(
            statusCol.$('span.co-icon-and-text span.co-icon-and-text span'),
