...
export const LocalVolumeSetInner: React.FC<LocalVolumeSetInnerProps> = (props) => {
  ...
  React.useEffect(() => {
    k8sList(NodeModel)
      ...
      .catch(() => dispatch({ type: 'setNodeNames', value: [] }));
  }, [ns, appName, dispatch]);
  ...
  const onMaxSizeChange = (size: string) => {
    if (size !== 'All' && (Number.isNaN(Number(size)) || Number(size) < state.minDiskSize)) {
      ...
      <Expandable toggleText="Advanced" data-test-id="create-lvs-form-advanced">
        ...
        >
          <Dropdown
            ...
            onChange={(mode: string) => dispatch({ type: 'setDiskMode', value: mode })}
          />
        </FormGroup>
        <FormGroup
          label="Disk Size"
