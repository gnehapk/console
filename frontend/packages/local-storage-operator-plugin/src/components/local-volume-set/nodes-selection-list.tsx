...
const getRows: GetRows = ({ componentProps, customData }, visibleRows, setVisibleRows, selectedNodes, setSelectedNodes) => {
  ...
  const { filteredNodes } = customData;

  const nodeList = filteredNodes?.length ? filteredNodes : data.map(getName);
  const filteredData = data.filter((node: NodeKind) => !hasTaints(node) && nodeList.includes(getName(node)));
...
export const NodesSelectionList: React.FC<NodesSelectionListProps> = (props) => {
  ...
  const { onSelect, selectedRows: selectedNodes,updateSelectedRows: setSelectedNodes } = useSelectList<NodeKind>(
    props.data,
    visibleRows,
  ...
  return (
    <>
      <div className={`lso-node-selection-table__table--scroll ${props.customData.className}`}>
        <Table
          {...props}
          aria-label="Select nodes for creating volume filter"
          data-test-id="create-lvs-form-node-selection-table"
          Header={getColumns}
          Rows={(rowProps) => getRows(rowProps, visibleRows, setVisibleRows, selectedNodes, setSelectedNodes)}
